...
interface RankingEntry extends UserProgressSummary {
  username: string
}
...
function ScoreStat({ icon, value, label }: { icon: ReactNode; value: string | number; label: string }) {
  return (
...
function RankingRow({ entry, rank, currentUsername }: { entry: RankingEntry; rank: number; currentUsername: string | null }) {
    ...
    >
      <div className="flex flex-wrap items-center gap-3 p-3 sm:gap-4 sm:p-4">
        ...
        <div className="grid w-full grid-cols-2 gap-2 sm:w-auto sm:min-w-[13rem]">
          <div className="border-2 px-3 py-2 text-center" style={{ borderColor: borderColor, background: 'var(--surface)' }}>
